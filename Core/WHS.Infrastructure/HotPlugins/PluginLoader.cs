...
using WHS.Infrastructure.HotPlugins.Internal;
using WHS.Infrastructure.HotPlugins.Loader;
...
namespace WHS.Infrastructure.HotPlugins
{
    ...
    public class PluginLoader : IDisposable
    {
        ...
        internal ManagedLoadContext _context;
        internal readonly AssemblyLoadContextBuilder _contextBuilder;
        ...
        internal void StartFileWatcher()
        {
            ...
            */
            ...
            _fileWatcher.Filter = Path.GetFileName(_config.MainAssemblyPath);//".dll";
        ...
        private static AssemblyLoadContextBuilder CreateLoadContextBuilder(PluginConfig config)
        {
            ...
            foreach (var ext in config.AdditionalProbingPaths)
            {
                builder.AddProbingPath(ext);
            }
            foreach (var ext in config.ResourceProbingPaths)
            {
                builder.AddResourceProbingPath(ext);
            }
            foreach (var ext in config.ResourceProbingSubpaths)
            {
                builder.AddResourceProbingSubpath(ext);
            }
