...
using WHS.Infrastructure.HotPlugins.LibraryModel;

namespace WHS.Infrastructure.HotPlugins.Loader
{
    /// <summary>
    ...
    internal class ManagedLoadContext : AssemblyLoadContext
    {
        ...
        protected override Assembly? Load(AssemblyName assemblyName)
        {
            ...
            if (!string.IsNullOrEmpty(assemblyName.CultureName) && !string.Equals("neutral", assemblyName.CultureName))
            {
                ...
                {
                    var resourcePath = Path.Combine(resourceRoot, "", assemblyName.Name + ".dll");
        ...
        private void OnUnloaded(AssemblyLoadContext obj)
        {
            //删除资源文件在WPFLocalizeExtension的缓存
            var sigle = WPFLocalizeExtension.Providers.ResxLocalizationProvider.Instance;
            string filename = Path.GetFileNameWithoutExtension(_mainAssemblyPath);
            sigle.TryRemoveAssembly(filename + ".Strings.resources");
