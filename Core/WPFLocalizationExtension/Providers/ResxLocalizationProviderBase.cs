        ...
        protected ResourceManager GetResourceManager(string resourceAssembly, string resourceDictionary)
        {
            ...
            if (!TryGetValue(resManKey, out var resManager))
            {
                ...
                if (assembly == null)
                {
                    try
                    {
                        //修改为获取最新的资源程序集，（防止app domain读取卸载的资源）
                        assembly = AppDomain.CurrentDomain.GetAssemblies().Where(w => new AssemblyName(w.FullName).Name == resourceAssembly).LastOrDefault();
